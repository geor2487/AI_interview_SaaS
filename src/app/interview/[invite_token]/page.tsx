...
export default function InviteGatewayPage() {
  ...
  useEffect(() => {
    const inviteToken = params.invite_token as string;
    if (!inviteToken) {
    ...
    const handleRedirect = async () => {
      // API経由で面接を検索（RLS回避）
      const res = await fetch(`/api/interviews/by-token?token=${inviteToken}`);
      if (!res.ok) {
        setError("このリンクは無効です。招待メールを確認してください。");
        return;
      }
      ...
      // 候補者サインアップページへリダイレクト
      // ログイン済みの場合はサインアップページ側でポータルに転送
      router.replace(`/candidate-signup?token=${inviteToken}`);
    };

    handleRedirect();
  }, [params.invite_token, router]);
