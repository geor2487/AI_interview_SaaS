...
export default function ProfilePage() {
  const [careers, setCareers] = useState<Career[]>([
    {
      ...
      periodStart: "2022-04",
      periodEnd: "2025-12",
      description: "React/Next.jsを用いたWebアプリケーション開発をリードしました。",
    },
  ]);
  ...
  const [certs, setCerts] = useState(["基本情報技術者試験", "TOEIC 850"]);
    ...
    <div className="space-y-8 pb-12">
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" defaultValue="田中 花子" className={inputClass} />
            </div>
            <div>
              <label className={labelClass}>生年月日</label>
              <input type="date" defaultValue="1998-05-15" className={inputClass} />
            </div>
            <div>
              <label className={labelClass}>性別</label>
              <select defaultValue="female" className={inputClass}>
            ...
            <div>
              ...
              <input type="tel" defaultValue="090-1234-5678" className={inputClass} />
            </div>
            <div className="md:col-span-2">
              <label className={labelClass}>居住地</label>
              <input type="text" defaultValue="東京都渋谷区" className={inputClass} />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-surface rounded-xl border border-border p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button
            onClick={addCareer}
            className="flex items-center gap-1.5 text-sm font-medium text-accent hover:text-accent-text transition-colors"
          >
          ...
          {careers.map((career) => (
            <div key={career.id} className="rounded-lg border border-border-sub bg-background p-4 space-y-3 relative group">
              <button
                ...
                className="absolute top-3 right-3 h-7 w-7 rounded-md flex items-center justify-center text-text-muted hover:text-red hover:bg-red-bg transition-colors opacity-0 group-hover:opacity-100"
              >
          ...
          {skills.map((skill) => (
            <div key={skill.id} className="flex items-center gap-3 group">
              ...
              <button
                ...
                className="h-8 w-8 rounded-md flex items-center justify-center text-text-muted hover:text-red hover:bg-red-bg transition-colors opacity-0 group-hover:opacity-100"
              >
      ...
      {/* 資格 */}
      <section className="bg-surface rounded-xl border border-border p-6">
        <h2 className={sectionTitle}>資格</h2>
        <div className="space-y-2">
          {certs.map((cert, i) => (
            <div key={i} className="flex items-center gap-3 group">
              <input type="text" defaultValue={cert} className={cn(inputClass, "flex-1")} />
              <button
          ...
          <button
            onClick={() => setCerts((prev) => [...prev, ""])}
            className="flex items-center gap-1.5 text-sm font-medium text-accent hover:text-accent-text transition-colors mt-2"
          >
            <Plus className="h-4 w-4" />
            追加
          </button>
      ...
      <section className="bg-surface rounded-xl border border-border p-6">
        ...
        <textarea
          rows={5}
          defaultValue="フロントエンド開発を5年間経験し、React/TypeScriptを用いたモダンなWebアプリケーション開発に強みを持っています。チームリードとしてプロジェクトを成功に導いた経験もあります。"
      ...
      <section className="bg-surface rounded-xl border border-border p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" defaultValue="フロントエンドエンジニア" className={inputClass} />
          </div>
          <div>
            <label className={labelClass}>希望年収</label>
            <input type="text" defaultValue="600万円 ~ 800万円" className={inputClass} />
          </div>
          <div>
            <label className={labelClass}>入社可能日</label>
            <input type="date" defaultValue="2026-04-01" className={inputClass} />
          </div>
        </div>
      </section>
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between rounded-lg border border-border-sub bg-background px-4 py-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-foreground">履歴書_田中花子.pdf</p>
